...
export default function AddressSearchModal({ isOpen, onClose, onComplete }) {
    ...
    useEffect(() => {
                    ...
                    if (window.naver?.maps?.Service) {
                        try {
                            ...
                            if (response.v2?.addresses?.length > 0) {
                                ...
                                result.lat = parseFloat(item.y)
                                result.lng = parseFloat(item.x)
                            }
                        } catch (error) {
                            console.error('[AddressSearchModal] Geocode 실패:', error)
                            // 좌표 없어도 주소는 반환 (수동 입력 허용)
                        }
                    }
        ...
        if (existing) {
            existing.addEventListener('load', loadPostcode, { once: true })
            return
        }
