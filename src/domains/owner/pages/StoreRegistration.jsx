...
export default function StoreRegistration({ initialData, onBack, onDone }) {
  ...
  const onBizFile = (e) => { const f = e.target.files?.[0]; if (!f) return; setBizFile(f); setBizPreview(URL.createObjectURL(f)) }
  const onCertFile = (cert, e) => { const f = e.target.files?.[0]; if (!f) return; setCertFiles((p) => ({ ...p, [cert]: { file: f, preview: URL.createObjectURL(f) } })) }

  const openPostcode = () => {
    const load = () => new window.daum.Postcode({
      oncomplete: (data) => setForm((f) => ({ ...f, road_name: data.roadAddress, lot_num: data.jibunAddress, si: data.sido, emd: data.bname })),
    }).open()
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      await OwnerApi.submitRegistration({
        ...
        initialStoreId: isClaim ? initialData.id : null,
      })
